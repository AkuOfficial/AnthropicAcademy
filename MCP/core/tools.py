...
class ToolManager:
    ...
    @classmethod
    async def get_all_tools(cls, clients: dict) -> list[types.Tool]:
        functions = []
    ...
    @classmethod
    async def execute_tool_requests(cls, clients: dict, response) -> list[types.Part]:
        tool_results = []
        ...
        for part in response.candidates[0].content.parts:
            if fn := part.function_call:
                ...
                try:
                    tool_output = await client.call_tool(mcp_tool_name, fn.args)
                    result = {"content": "\n".join([item.text for item in tool_output.content])}
